...
source_name="Android Open Source Andromeda Project" #Name to display in changelog.md top before version
# input variables end
...
echo -e "";
# use 'export days_to_log=5' before '. build/envsetup.sh' where 5 is days to log
read -r -t 30 days_to_log || days_to_log=7
fi
echo >> $Changelog;
echo "$source_name Changelog"    >> $Changelog;
echo '' >> $Changelog;
echo >> $Changelog;
